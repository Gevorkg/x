...
class UpdateUserInfoBloc
  ...
  UpdateUserInfoBloc({required UserRepository userRepository})
    ...
    on<UploadPicture>((event, emit) async {
      ...
      try {
        String userImage =
            await _userRepository.uploadAvatarPicture(event.file, event.userId);
    ...
    on<UploadBackgroundPicture>((event, emit) async {
      try {
        String userBackgroundImage = await _userRepository
            .uploadBackgroundPicture(event.file, event.userId);
